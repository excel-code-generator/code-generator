...
public class DdlSqlServerTranslatorImpl extends BaseDdlTranslator {
    ...
    private String genForeignKey() throws CodeGenException {
        ...
        for (ForeignModel model : this.foreignKeyList) {
            ...
            StringBuilder foreignKeyName = new StringBuilder(String.format("FK_%s_%s_ON",
            ...
            for (ForeignDetailModel foreignColumns : model.getForeignColumns()) {
                ...
                foreignKeyName.append("_").append(foreignColumns.getDdlDetail().getColName());
            }

            // 超过128字符时截断
            if (foreignKeyName.length() > 128) {
                sb.append(String.format(Resources.getString("E_012"), foreignKeyName.toString()));
                foreignKeyName = new StringBuilder(foreignKeyName.substring(0, 128));
            }
